...
def summarize_performance(step, g_model, c_model, latent_dim, dataset, n_samples=1000):
    ...
    X, y = dataset
    #pred = c_model.predict(X, verbose=0)
    #acc = tf.keras.metrics.CategoricalAccuracy()
    _, acc = c_model.evaluate(X, y.argmax(1), verbose=0)
    wandb.log({"val_accuracy": acc}, step = step)
...
def train(g_model, d_model, c_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=512):
    ...
    for i in range(n_steps):
        ...
        if (i+1) % (bat_per_epo * 1) == 0:
            summarize_performance(step, g_model, c_model, latent_dim, dataset)
            step += 1
...
config.model = "SGAN Discriminator ResNetV2"
config.dataset = "CIFAR10H"
config.loss = "sparse_categorical_crossentropy"
config.optimizer = "Adam"
config.Dropout = 0.4
config.lrdecay = 0.01
wandb.run.name = "SGAN ResNet 100 per class"
wandb.run.save()
